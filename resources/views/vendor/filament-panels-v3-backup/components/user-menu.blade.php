            ...
            <x-filament::dropdown.list>
                ...
                >
                    <div class="grid gap-0.5">
                        <div class="font-medium">{{ $profileItem?->getLabel() ?? ($profilePage ? $profilePage::getLabel() : null) ?? filament()->getUserName($user) }}</div>
                        <div class="text-xs text-gray-500 dark:text-gray-400 font-normal truncate">{{ $user->tenant?->name }}</div>
                    </div>
                </x-filament::dropdown.list.item>
            </x-filament::dropdown.list>
        @else
            ...
            >
                <div class="grid gap-0.5">
                    <div class="font-medium">{{ $profileItem?->getLabel() ?? filament()->getUserName($user) }}</div>
                    <div class="text-xs text-gray-500 dark:text-gray-400 font-normal truncate">{{ $user->tenant?->name }}</div>
                </div>
            </x-filament::dropdown.header>
        @endif
